...
<template>
  <div class="qm-store-class-base">
    <div class="search-form mb20">
      <p class="c-gray mb5 mt20">条形码可手动输入，也可以使用巴枪扫码输入</p>
      <p class="c-gray mb5">平台若开启商品审核，则商品新增/编辑后将自动到仓库中的待审核列表</p>
      <p class="c-gray mb5 ">可制作海报，用户扫码即可跳转对应商品详情页进行推广宣传&nbsp;&nbsp;<a class="orange" @click="$router.push('/promotion/store_poster/index')">前往制作>></a></p>
    ...
    <div class="operate-bar mt20" v-if="!!pageParams.total">
      ...
      <a href="javascript:;" class="ncsc-btn-mini mr5" @click="openunShowDialog(1)">下架</a>
        ...
        <table class="ncsc-table mt20">
          ...
          <tr v-for="(item,i) in warn_list" :key="item.goods_commonid">
            ...
            <td class="handle">
              ...
              <p><a href="javascript:;" class="ncsc-btn" @click="copyLink(item)">复制链接</a></p>
            </td>
          </tr>
          </tbody>
        </table>
    ...
    <qm-tips :tipsVisible.sync="delDialog" :width="'420px'"  @close="delDialog = false" @confirm="confirmDel()">
      ...
      <template slot="content">
        您确定要删除吗?
        <p v-if="groupName.length > 0">一旦删除，以下组合销售将一并失效:<br>{{groupName.join('、')}}</p>
      </template>
    </qm-tips>
    <qm-tips :tipsVisible.sync="unShowDialog" :width="'420px'"  @close="unShowDialog = false" @confirm="goodsUnShow()">
      <template slot="title">提示信息</template>
      <template slot="content">
        您确定要下架商品吗?
        <p v-if="groupName.length > 0">一旦下架，以下组合销售将一并失效:<br>{{groupName.join('、')}}</p>
      </template>
    </qm-tips>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      copyLink(item) {
        var url = 'gotoshenbd://share.shenbd.com/share.html?type=goods&gid='+item.goods_id+'&cid='+item.goods_commonid+'&sid='+item.store_id;
      ...
      openunShowDialog(type,item) {
        ...
        this.unShowDialogType = type;
        storeGoodsOnline.getGroupGoods({commonId:commonId}).then(res=>{
          this.groupName = res.bundlings;
          this.unShowDialog = true;
        })
      },
      async goodsUnShow() {
        let res;
        if(this.unShowDialogType === 1) {//批量下架
          res = await storeGoodsOnline.goodsUnshow({commonId:this.checkState.join(',')})
        }else{
          res = await storeGoodsOnline.goodsUnshow({commonId:this.goods.goodsCommonId})
        }
        if(res.resultCode === 1) {
          util.msg('商品下架成功');
          this.unShowDialog = false;
          var newPageParams = _.clone(this.pageParams);
          if(this.unShowDialogType === 1) {
      ...
      getPageData(data){
        this.beforeGetPage();
        storeGoodsOnline.geOnlineGoodsList(this.pageParams).then((res)=>{
