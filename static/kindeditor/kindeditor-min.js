  ...
  function(window, undefined) {
    ...
    function _getInitHtml(a, b, c, d) {
      ...
      d && e.push("<style>" + d + "</style>"),
        e.push("<script></script></head><body " + (b ? 'class="' + b + '"': "") + "></body></html>"),
        e.join("\n")
    }
    ...
    if (!window.KindEditor) {
        ...
        _extend(KWidget, {
          ...
          autoPos: function(a, b) {
            ...
            if (c._alignEl) {
              var g = K(c._alignEl),
                h = g.pos(),
                i = _round(g[0].clientWidth / 2 - d / 2),
                j = _round(g[0].clientHeight / 2 - e / 2);
              x = 0 > i ? h.x: h.x + i,
                y = 0 > j ? h.y: h.y + j
            } else {
              var k = _docElement(c.doc);
              x = _round(f.x + (k.clientWidth - d) / 2),
                y = _round(f.y + (k.clientHeight - e) / 2)
            }
            return _IE && 7 > _V || _QUIRKS || (x -= f.x, y -= f.y),
              c.pos(x, y)
          },
          remove: function() {
            var a = this;
      ...
      KEditor.prototype = {
        ...
        resize: function(a, b, c) {
          var d = this;
          return c = _undef(c, !0),
          a && (/%/.test(a) || (a = _removeUnit(a), a = a < d.minWidth ? d.minWidth: a), d.container.css("width", _addUnit(a)), c && (d.width = _addUnit(a))),
          b && (b = _removeUnit(b), editHeight = _removeUnit(b) - d.toolbar.div.height() - d.statusbar.height(), editHeight = editHeight < d.minHeight ? d.minHeight: editHeight, d.edit.setHeight(editHeight), c && (d.height = _addUnit(b))),
            d
        },
        select: function() {
  ...
  KindEditor.plugin("multiimage",
    function(a) {
      var b = this,
        ...
        f = a.undef(b.imageSizeLimit, "4MB"),
  ...
  function() {
    window.SWFUpload = function(a) {
      ...
      SWFUpload.completeURL = function(a) {
        ...
        var b = (window.location.protocol + "//" + window.location.hostname + (window.location.port ? ":" + window.location.port: ""), window.location.pathname.lastIndexOf("/"));
